...
const ForgotPassword = ({navigation}) => {
  ...
  const sendOTP = async (values, {setSubmitting, setErrors}) => {
    setSpinner({status:true,message:"OTP Sending..."})
    try {
      const response = await axios.post(
        `${localpath}/api/users/forgot-password`,
        {email: values.email},
      );
      ...
      ToastAndroid.show(
        error?.response?.data?.message || 'Error in Sending OTP',
        ToastAndroid.SHORT,
  ...
  const verifyOTPAndChangePassword = async (
    ...
    setSpinner({status:true,message:"OTP Verifying..."})
    try {
      const response = await axios.post(
        `${localpath}/api/users/update-password`,
